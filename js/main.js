...
function initLiveChat() {
  // Check if Tawk.to script was loaded (user has connected it)
  // If Tawk_API is defined, we don't render the placeholder button since Tawk.to manages its own widget
  setTimeout(() => {
    if (window.Tawk_API) {
    ...
    btn.setAttribute('aria-label', 'Open Live Chat Demo');
    btn.innerHTML = `
    ...
    popup.innerHTML = `
      <div class="chat-popup-header">
        <div class="chat-popup-title">
          ...
          LeadSprint Autopilot
        </div>
        <button class="chat-popup-close" aria-label="Close message">✕</button>
      </div>
      <div class="chat-popup-body">
        <p style="margin-bottom: 0.75rem;"><strong>Live Chat Connected (Demo Mode)</strong></p>
        <p style="font-size: 0.85rem; color: var(--text-muted); margin-bottom: 1rem;">
          To enable live customer chat, sign up at <strong>tawk.to</strong> and replace this placeholder with your script inside the footer of <code>index.html</code> and <code>contact.html</code>.
        </p>
        <div style="background: rgba(255,255,255,0.01); border: 1px solid var(--border-light); padding: 0.5rem; border-radius: var(--border-radius-sm); font-family: monospace; font-size: 0.75rem;">
...
function initAnnouncementBanner() {
  ...
  if (sessionStorage.getItem('leadsprint_banner_closed') === 'true') {
  ...
  banner.innerHTML = `
    <a href="contact.html" class="banner-link-wrapper">
      <span class="banner-text">🚀 Limited Time Offer — Get Your FREE 5-Day AI Lead System Trial. We install everything in 48 hours. Book your free strategy call today.</span>
    </a>
  ...
  // Run immediately and on resize/load events
  updateBannerHeight();
  window.addEventListener('resize', updateBannerHeight, { passive: true });
  window.addEventListener('load', updateBannerHeight, { passive: true });
  ...
  closeBtn.addEventListener('click', (e) => {
    ...
    // Slide up banner smoothly
    banner.style.transform = 'translateY(-100%)';
    banner.style.opacity = '0';

    // Remove document custom property height
    document.documentElement.style.setProperty('--banner-height', '0px');

    // Save state
    sessionStorage.setItem('leadsprint_banner_closed', 'true');
